...
class _CategoryProductState extends State<CategoryProduct> {
  ...
  void getCategoryPage(dynamic catid, dynamic storeid) async {
    print('page -> $page');
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var http = Client();
    http.post(catProductUri, body: {
      ...
      if (value.statusCode == 200) {
        ...
        if ('${data1.status}' == '1') {
          setState(() {
            page = page+1;
            products.addAll(List.from(data1.data));
          });
        }
      }
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    if (!enterFirst) {
      ...
      catid = receivedData['cat_id'];
      storedetail = receivedData['storedetail'];
      print(store_id);
      print(catid);
      getWislist(store_id);
      getCategory(catid, store_id);
    }
    return Scaffold(
      ...
      body: BlocBuilder<CartListProvider, List<CartItemData>>(
        builder: (context, cartList) {
          ...
          return Container(
            ...
            child: (isLoading)
                ? buildGridShView()
                : (products!=null && products.length>0)?buildGridView(
                products, wishModel, '$apCurrency', storedetail,locale):Align(
