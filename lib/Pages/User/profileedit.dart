...
class _ProfileEditState extends State<ProfileEdit> {
  ...
  List<CityDataBean> cityList = [CityDataBean('--','Select city')];
  List<StateDataBean> socityList = [StateDataBean('--','Select socity/area','--','--')];
  CityDataBean cityData = CityDataBean('--','Select city');
  StateDataBean socityData = StateDataBean('--','Select socity/area','--','--');
  ...
  void getProfileValue() async{
    ...
    setState(() {
      ...
      dynamic city_name = preferences.getString('city_name');
      dynamic society_name = preferences.getString('society_name');
      ...
      cityData = CityDataBean(user_city,city_name);
      cityList.add(cityData);
      socityData = StateDataBean(user_area,society_name,'','');
      ...
      if (value.statusCode == 200) {
        ...
        if (data1.status == "1" || data1.status == 1) {
          ...
          setState(() {
            ...
            socityData = StateDataBean('--','Select socity/area','--','--');
          });
        }
      }
      setState(() {
                      ...
                      child: DropdownButton<CityDataBean>(
                        ...
                        items: cityList.map((value) {
                          return DropdownMenuItem<CityDataBean>(
                            value: value,
                            child: Text((value!=null && value.city_name!=null)?value.city_name:'--',
                                overflow: TextOverflow.clip),
                          );
                        }).toList(),
                        onChanged: (value) {
                      ...
                      child: DropdownButton<StateDataBean>(
                        ...
                        items: socityList.map((value) {
                          return DropdownMenuItem<StateDataBean>(
                            value: value,
                            child: Text((value!=null && value.society_name!=null)?value.society_name:'--',
                                overflow: TextOverflow.clip),
                          );
                        }).toList(),
                        onChanged: (valued) {
  ...
  void hitSignUpUrl(
    ...
    print('$user_name $user_email $user_phone $user_city $user_area ${prefs.getInt('user_id')}');
    print('$token');
    if (token != null) {
      String fid;
      if(_image!=null){
        ...
        http.MultipartFile.fromPath('user_image', _image.path, filename: fid)
          ...
          requestMulti.send().then((values) {
            values.stream.toBytes().then((value) {
              ...
              var jsD = jsonDecode(jsonData.toString());
              if('${jsD['status']}'=='1'){
                prefs.setString("user_name", '$user_name');
                prefs.setString("user_email", '$user_email');
                prefs.setString("user_image", '${jsD['user_image']}');
                prefs.setString("user_phone", '$user_phone');
                prefs.setString("user_city", '${cityData.city_id}');
                prefs.setString("user_area", '${socityData.society_id}');
                prefs.setString("city_name", '${cityData.city_name}');
                prefs.setString("society_name", '${socityData.society_name}');
              }
